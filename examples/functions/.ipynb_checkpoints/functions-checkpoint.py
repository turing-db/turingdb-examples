...
def create_graph_from_df(
    ...
    *,
    ...
    target_info = process_node_info(target_node_col) if target_node_col is not None else None
    ...
    if node_attributes_df is not None:
        ...
        unique_node_attrs = node_attributes_df.drop_duplicates(subset=[node_attributes_key_col])
        node_attrs_lookup = unique_node_attrs.set_index(node_attributes_key_col).to_dict('index')
    
    # Function to add a node to the graph with its attributes
    def add_node_with_attrs(row, node_id_col, node_label_col, node_type_val, is_type_column, attr_cols, node_prefix=''):
...
def build_create_command_from_networkx(G, edge_type_key='type'):
    ...
    for source, target, edge_attrs in G.edges(data=True):
        ...
        filtered_edge_attrs = {k: v for k, v in edge_attrs.items() if k != edge_type_key}
        
        # Build edge properties string
        edge_props = ", ".join([f'"{k}":"{escape_value(v)}"' for k, v in filtered_edge_attrs.items()]) if filtered_edge_attrs else ""
        ...
        edge_parts.append(f"({source_var})-[:{relationship_type}{edge_props_str}]-({target_var})")
...
def show_link(link_str=None, text='Click to visualise graphs in Turing UI'):
    # Create the HTML with the href dynamically using the variable
    display(HTML(f"""
    <div class="alert alert-block alert-success">
        <p style="font-size:20px; ">
            <b><a href="{link_str}" target="_blank">{text}</a></b>
        </p>
    </div>
    """))